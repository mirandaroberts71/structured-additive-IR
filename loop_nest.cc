  ...
  mlir::LogicalResult VerifyLoopsOpened(
    ...
    for (mlir::Attribute loop : loops) {
      ...
      return mlir::emitError(loc)
             << "loop " << loop << " must open at or before this operation";
    }
    return mlir::success();
  }
...
static mlir::LogicalResult VerifyLoopsOpened(
  ...
  for (mlir::Value dimension : op.domain()) {
    ...
    if (mlir::failed(loop_nest_state.VerifyLoopsOpened(constraints.open_loops,
                                                       op.getLoc()))) {
      return mlir::failure();
    }
  }
...
mlir::LogicalResult VerifyLoopNests(SairProgramOp program) {
  ...
  result = program.walk([&](ComputeOp op) -> mlir::WalkResult {
    ...
    return VerifyLoopsOpened(cast<SairOp>(op.getOperation()), loop_nest_state,
  ...
  result = program.walk([&](SairOp op) -> mlir::WalkResult {
    return VerifyDependencies(op, loop_nest_analysis,
                              loop_constraints_analysis);
  });
  if (result.wasInterrupted()) return mlir::failure();
